...
add $t3, $zero, $zero # t3 will be used to store 4r*cur
...
# Loop condition: s4 != s2, which means not reaching the right-end
beq $s4, $s2, u_loop_Ex
u_loop:
...
add $s5, $t2, $a0 # s5 = 4 * cur + a0, leftmost
sub $s6, $s4, $t2 # s6 = s4 - 4 * cur, topmost

# Loop condition: s6 != s4, which means not reaching the current element
beq $s6, $s4, u_sub_dot_Ex
u_sub_dot:
...
# L loop
addi $s4, $s1, 4 # s4 starts from the bottom element of the diagonal element

# Loop condition: s4 != s3, which means not reaching the diagonal element
beq $s4, $s3, l_loop_Ex
l_loop:
...
# Sub dot product result from f0 loop
sub $s5, $s4, $t3 # s5 = s4 - 4 * r * cur, the left most
add $s6, $t3, $a0 # s6 = matrix_begin_addr + 4 * r * cur, the top most

# Loop condition: s5 != s4, which means not reaching current element
beq $s5, $s4, l_sub_dot_Ex
l_sub_dot:
...
add $t3, $t3, $t0 # Store 4r * cur in $t3 <=> t3 = t3 + 4r for each step

addi $s2, $s2, 4 # move right_end down 1 element
add $s3, $s3, $t0 # move bottom_end right 1 element

add $s1, $s1, $t1 # s1 = s1 + 4(r+1), to move to the next diagonal element
addi $s0, $s0, 1
